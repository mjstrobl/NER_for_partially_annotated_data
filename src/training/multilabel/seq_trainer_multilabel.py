...
def categorical_cross_entropy(y_pred, y_true,weights):
    y_pred = torch.clamp(y_pred, 1e-9, 1 - 1e-9)
    return -(y_true * torch.log(y_pred)).multiply(weights).sum(dim=1).mean()

def train(results_dir,original_output_dir, max_steps, save_steps, evaluate_during_training, datasets, model, tokenizer, label_list, num_labels, per_gpu_train_batch_size, gradient_accumulation_steps, num_train_epochs, weight_decay, learning_rate, adam_epsilon, warmup_steps, model_name_or_path, device, model_type, max_grad_norm, logging_steps):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            a = batch[3].view(-1, num_labels)
            b = logits.view(-1, num_labels)
            c = batch[4].type_as(logits).view(-1, num_labels)

            # loss_func = BCEWithLogitsLoss(weight=a,pos_weight=torch.tensor(datasets['train_pos_weight']).to(device))
            loss_func = BCEWithLogitsLoss(weight=a)
            loss = loss_func(b, c)
        ...
        if (evaluate_during_training):  # Only evaluate when single GPU otherwise metrics may not average well
            evaluate(results_dir, datasets, model, per_gpu_train_batch_size, device, model_type, label_list, prefix='dev')
            #evaluate(results_dir, datasets, model, per_gpu_train_batch_size, device, model_type,label_list, prefix='gold')
...
def evaluate(results_dir,datasets, model, per_gpu_eval_batch_size, device, model_type, label_list,prefix="dev"):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        if raw_preds is None:
            ...
            weights = current_weights.detach().cpu().numpy()
        else:
            raw_preds = np.append(raw_preds, logits.detach().cpu().numpy(), axis=0)
            out_label_ids = np.append(out_label_ids, labels.detach().cpu().numpy(), axis=0)
            weights = np.append(weights, current_weights.detach().cpu().numpy(), axis=0)

    eval_loss = eval_loss / nb_eval_steps
    preds = np.argmax(raw_preds, axis=2)

    weights = np.zeros((out_label_ids.shape[0],out_label_ids.shape[1]))
    for i in range(out_label_ids.shape[0]):
        for j in range(out_label_ids.shape[1]):
            if np.max(out_label_ids[i,j]) == 1.0:
                weights[i,j] = 1
            else:
                weights[i,j] = 0


    out_label_ids = np.argmax(out_label_ids, axis=2)
    ...
    filename = results_dir + prefix + '_multilabel.txt'
    with open(filename,'w') as f:
        for i in range(out_label_ids.shape[0]):
            ...
            correct = True
            for j in range(out_label_ids.shape[1]):
                #if np.sum(out_label_ids[i, j]) > 0:
                #if np.sum(weights[i][j]) > 0:
                #if j < len(tokens) and not tokens[j].startswith('#') and not tokens[j] == '[SEP]' and not tokens[j] == '[CLS]':
                if weights[i,j] == 1:
