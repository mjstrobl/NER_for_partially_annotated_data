...
from datasets_singlelabel import conll_file_to_features,get_labels, convert_examples_to_features, read_examples_from_file_whole_lines,convert_wiki_gold_features
import numpy as np
...
import torch
import random
from seq_trainer_singlelabel import train,evaluate
...
def main():
    ...
    #print("Found " + str(more_entities) + " more entities.")

    features = convert_wiki_gold_features(config['wiki_gold_filename'], label_list, max_seq_length, tokenizer, pad_token_label_id=pad_token_label_id)
    ...
    for mode in modes:
        ...
        dataset_wiki = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids)
    ...
    if do_eval:
        ...
        print("ALL:")
        evaluate(config['results_dir'], datasets, model, per_gpu_train_batch_size, device, model_type, pad_token_label_id, label_list,prefix='dev')
    ...
    if do_predict:
        print("*** Predict ***")
        print("ALL:")
        evaluate(config['results_dir'], datasets, model, per_gpu_train_batch_size, device, model_type, pad_token_label_id, label_list,prefix='test')
